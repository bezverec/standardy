...
export function registryImportSql(registry: NormalizedRegistry): string {
  const statements = [
    ...
    "DELETE FROM national_standards;",
  ...
  for (const nationalStandard of registry.national_standards) {
    statements.push(`INSERT INTO national_standards (id, title_cs, status, data_json) VALUES (${sqlText(nationalStandard.id)}, ${nullable(nationalStandard.title.cs)}, ${sqlText(nationalStandard.status)}, ${json(nationalStandard)});`);
  }
  ...
  for (const rule of registry.rules) {
    statements.push(`INSERT INTO rules (id, national_standard_id, title_cs, source_file, data_json) VALUES (${sqlText(rule.id)}, ${sqlText(rule.national_standard.id)}, ${nullable(rule.title.cs)}, ${nullable(rule.source_file)}, ${json(rule)});`);
  }
  for (const version of registry.rule_versions) {
    ...
    statements.push(`INSERT INTO rule_versions (rule_id, version, national_standard_id, standard_id, target_id, category, severity, status, verification_status, search_text, data_json) VALUES (${sqlText(version.rule_id)}, ${sqlText(version.version)}, ${sqlText(version.national_standard_id)}, ${nullable(standardId)}, ${sqlText(version.target.entity)}, ${sqlText(version.category)}, ${sqlText(version.severity)}, ${sqlText(version.status)}, ${sqlText(version.verification.status)}, ${sqlText([version.rule_id, version.title.cs, version.description?.cs, version.target.entity, JSON.stringify(version.requirement), version.interpretation?.cs, JSON.stringify(version.source), JSON.stringify(version.implementations)].filter(Boolean).join(" "))}, ${json(version)});`);
  }
